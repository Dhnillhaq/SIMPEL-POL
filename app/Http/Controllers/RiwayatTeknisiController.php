...
class RiwayatTeknisiController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = Perbaikan::with(['periode', 'inspeksi', 'inspeksi.fasilitas'])
            ...
            ->whereHas('inspeksi', function ($query) {
                $query->where('id_user_teknisi', auth()->id());
            });
        ...
        if ($request->ajax()) {
            $html = view('teknisi.riwayat.riwayat_table', compact('perbaikan'))->render();
            return response()->json(['html' => $html]);
        }


        return view('teknisi.riwayat.index', compact('breadcrumb', 'page', 'activeMenu', 'perbaikan', 'periode'));
    }
    ...
    public function show_ajax(Perbaikan $perbaikan)
    {
        ...
        // return view('teknisi.riwayat.detail', compact('aduan', 'biaya', 'perbaikan', 'fasilitas', 'avgRating', 'jumlahAduan'))->render();

        // $aduan = $perbaikan->aduan_tertangani;

        return view('teknisi.riwayat.detail')->with([
            'aduan' => $perbaikan->aduan_tertangani,
            'perbaikan' => $perbaikan
        ]);
    }
    ...
    public function comment_ajax(Perbaikan $perbaikan)
    {
        ...
        return view('teknisi.riwayat.comment')->with([
            'aduan' => $perbaikan->aduan_tertangani,
            'perbaikan' => $perbaikan,
        ]);
    }
